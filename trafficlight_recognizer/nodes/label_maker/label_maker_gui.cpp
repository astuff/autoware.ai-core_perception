...
LabelMakerGui::LabelMakerGui(QWidget* parent) : QMainWindow(parent), ui_(new Ui::LabelMakerGui)
{
  // Initialize GUI
  ui_->setupUi(this);
  ...
  connect(ui_->image_id_horizontal_slider_, SIGNAL(valueChanged(int)), ui_->image_id_spin_box_, SLOT(setValue(int)));

  connect(ui_->image_id_spin_box_, SIGNAL(valueChanged(int)), ui_->image_id_horizontal_slider_, SLOT(setValue(int)));

  connect(ui_->image_id_spin_box_, SIGNAL(valueChanged(int)), this, SLOT(ShowImage()));

  connect(ui_->radio_button_group_, SIGNAL(buttonClicked(QAbstractButton*)), this,
          SLOT(SetRadioButtonsColor(QAbstractButton*)));

  connect(ui_->next_push_button_, SIGNAL(pressed()), this, SLOT(SaveAndGoNext()));

  connect(ui_->previous_push_button_, SIGNAL(pressed()), this, SLOT(SaveAndGoPrevious()));

  connect(ui_->reset_push_button_, SIGNAL(pressed()), this, SLOT(ResetSelection()));
...
QString LabelMakerGui::GetTargetDirectoryPath()
{
  ...
  while (true)
  {
    path = QFileDialog::getExistingDirectory(this, "Select Existing Dataset Directory");
    if (path == "")
    {
    ...
    else
    {
      ...
      error_message = "\"Images\" direcotry cannnot be found in \"" + path + "\". \n\n" +
                      "Make sure DataSet directory surely exists. \n" +
                      "Or Execute \"rosrun trafficlight_recognizer roi_extractor\" first in order to create dataset "
                      "source ";
...
bool LabelMakerGui::SaveCurrentState()
{
  ...
  if (selected == 0)
  {  // Show warning window as no state is selected
  ...
  file_system_operator_.WriteStateToFile("Images",  // Image file should be under "Image" directory
                                         image_list_[current_image_id], state, image_size.height(), image_size.width(),
                                         image_depth, start.x(), start.y(), end.x(), end.y());
  return true;
}
...
void LabelMakerGui::SaveAndGoNext()
{
  ...
  {
    QMessageBox::information(this, "Information", "This is the end of image list.\n Thank you for your great work!");
    return;
  }
...
void LabelMakerGui::SaveAndGoPrevious()
{
  ...
  {
    QMessageBox::information(this, "Information", "This is the first image of the list.");
    return;
  }
  ui_->image_id_spin_box_->setValue(image_id);
}
