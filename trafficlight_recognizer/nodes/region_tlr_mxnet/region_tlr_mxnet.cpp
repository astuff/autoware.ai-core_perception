...
void RegionTLRMxNetROSNode::RunRecognition()
{
  ...
  recognizer.Init((const char*)json_data.GetBuffer(), (const char*)param_data.GetBuffer(), param_data.GetLength(),
                  use_gpu_, gpu_id_);
...
void RegionTLRMxNetROSNode::StartSubscribersAndPublishers()
{
  ...
  image_subscriber = node_handle.subscribe(image_topic_name_, 1, &RegionTLRMxNetROSNode::ImageRawCallback, this);
  roi_signal_subscriber = node_handle.subscribe("/roi_signal", 1, &RegionTLRMxNetROSNode::ROISignalCallback, this);
