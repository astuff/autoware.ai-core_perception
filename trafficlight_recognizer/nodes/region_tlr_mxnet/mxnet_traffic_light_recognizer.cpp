...
void MxNetTrafficLightRecognizer::Init(const char* in_network_definition_buffer, const char* in_pretrained_model_buffer,
                                       int in_pretrained_model_length, const bool in_use_gpu,
                                       const unsigned int in_gpu_id)
{
  ...
  MXPredCreate(in_network_definition_buffer, in_pretrained_model_buffer, in_pretrained_model_length, dev_type, dev_id,
               num_input_nodes, input_keys, input_shape_indptr, input_shape_data, &prediction_handle_);
  assert(prediction_handle_);
}

void MxNetTrafficLightRecognizer::PreProcessImage(const cv::Mat& in_image, mx_float* out_image_data,
                                                  const int in_channels, const cv::Size in_resize_size)
{
  if (in_image.empty())
  {
...
LightState MxNetTrafficLightRecognizer::RecognizeLightState(const cv::Mat& in_image, double in_score_threshold)
{
  ...
  int highest_score_index =
      std::distance(output_data.begin(), std::max_element(output_data.begin(), output_data.end()));
